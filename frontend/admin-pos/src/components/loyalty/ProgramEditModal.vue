<!--
  Program Edit Modal Component
  Modal for editing existing loyalty programs with tiers and settings
-->
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <form @submit.prevent="updateProgram">
              ...
              <div class="space-y-3">
                ...
                <label class="flex items-center">
                  <input
                    ...
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                  <span class="ml-2 text-sm text-gray-900">Program is active</span>
                </label>
                <label class="flex items-center">
                  <input
            ...
            <!-- Program Statistics -->
            <div class="mb-6">
              <h4 class="text-md font-semibold text-gray-900 mb-4">Program Statistics</h4>
...
watch(() => props.program, (newProgram) => {
  if (newProgram) {
    populateForm(newProgram)
  }
}, { immediate: true })

// Methods
const populateForm = (program: LoyaltyProgram) => {
  form.value = {
    name: program.name,
    type: program.type || 'points',
    description: program.description,
    pointsPerDollar: program.pointsPerDollar,
    minPointsRedeem: program.minPointsRedeem,
    pointValue: program.pointValue || 1.0,
    ...
    allowPointsTransfer: program.allowPointsTransfer || false,
    tiers: program.tiers ? [...program.tiers] : []
  }
}
...
const addTier = () => {
  ...
  form.value.tiers.push({
    ...
    minSpent: form.value.tiers.length > 0 ? form.value.tiers[form.value.tiers.length - 1].minSpent + 500 : 0,
...
const updateProgram = async () => {
  if (!isFormValid.value || isUpdating.value || !props.program) return

  isUpdating.value = true

  try {
    // Call API to update program
    console.log('Updating program:', form.value)

    // Simulate API call
    await new Promise(resolve => setTimeout(resolve, 1500))

    emit('updated', props.program.id)
    closeModal()
